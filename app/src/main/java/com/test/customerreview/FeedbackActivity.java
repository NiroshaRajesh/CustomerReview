...
public class FeedbackActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    int customer_id = 0;
    int fit = -1, quality = -1, satisfaction =-1, future_buy =-1, rate = 0;
    ...
    private boolean feedback(int customer_id) {
        String query = "select * from BedCover where cid = ?";
    ...
    void setViewMode() {
        ...
        Cursor cursor = dbHelper.getData("BedCover", customer_id);
        if (cursor.getCount() > 0) {
            rate = Integer.parseInt(cursor.getString(1));
            fit = Integer.parseInt(cursor.getString(2));
            quality = Integer.parseInt(cursor.getString(3));
            satisfaction = Integer.parseInt(cursor.getString(4));
            future_buy = Integer.parseInt(cursor.getString(5));
            familiarity = cursor.getString(6);
        ...
        setRate(rate);
        if (fit > -1)
            ((RadioButton)binding.radioGroup1.getChildAt(fit)).setChecked(true);
        if (quality > -1)
            ((RadioButton)binding.radioGroup2.getChildAt(quality)).setChecked(true);
        if (satisfaction > -1)
            ((RadioButton)binding.radioGroup3.getChildAt(satisfaction)).setChecked(true);
        if (future_buy > -1)
            ((RadioButton)binding.radioGroup4.getChildAt(future_buy)).setChecked(true);
    ...
    private void saveData() {
        ...
        values.put("fitting", String.valueOf(fit));
        ...
        long row = db.insert("BedCover", null, values);
        ...
        if (row == -1)
            ...
            startActivity(new Intent(FeedbackActivity.this, HomeActivity.class).addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP | Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK));
            finish();
        }
    }
